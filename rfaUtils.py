...
def getLog(log_dir_name,script_filename):
    """
    ...
    try:
        ...
        log = open(os.path.join(log_dir, script_filename +  "_" + 
...
def getTestCases(test_run_id):
    ...
    for line in test_case_file:
        ...
        for i in range (0, len(test_case_keys_list)):
            ...
            # values of all keys except 'param_list' (test_case_keys_list[4]) are added to the dict as strings 
            if key != test_case_keys_list[4]:
                # insert key-value pair into 'test_case_insert_dict'. 'value' is saved as string
                if key not in test_case_insert_dict:
                    test_case_insert_dict[key] = value
                else:
                    return -1
            # value of 'param_list' (test_case_keys_list[4]) key is added as list
            else:
                # insert key-value pair into 'test_case_insert_dict'. 'value' is saved as list
                if key not in test_case_insert_dict:
                    test_case_insert_dict[key] = value.split(',')
                else:
                    return -1                    
